...
A buffer overflow occurs when a read from or write to a buffer exceeds the
boundaries of the allocation used for the buffer. This typically results in
adjacent data structures being accessed, which has the potential of leaking or
compromising the integrity of this adjacent data.

When the buffer is allocated on the stack, we talk about a stack buffer
overflow. In this section we focus on stack buffer overflows, since, in the
absence of any mitigations, they are some of the simplest buffer overflows to
exploit.

The stack frame of a function includes important control information, such as
the saved return address and the saved frame pointer. Overwriting these values
unintentionally will typically result in a crash, but the overflowing values
can be carefully chosen by an attacker to gain control of the program's
execution.
...
[^oversimplified]: This is an oversimplified example for illustrative purposes.
  ...
  vulnerabilities](https://cwe.mitre.org/data/definitions/121.html), there are
  [many real-world examples of stack buffer
  overflows](https://www.cvedetails.com/vulnerability-list/cweid-121/vulnerabilities.html).
...
compiler version we're using and the architecture we're compiling for.

As can be seen the stack diagram, an overflowing write can overwrite the saved
frame pointer (FP) and link register (LR), which contains the return address,
in `main`'s frame. This is an exploitable vulnerability.
...
overwrite control data is going to overwrite the canary first, a corruption of
the control data through a stack buffer overflow will be detected.
...
integrity of control data against any modification.

Since stack canaries come with additional checks, compilers usually employ
heuristics to determine which functions are considered vulnerable and therefore
include the mitigation in their generated code. Since heuristics aren't always
perfect, this poses another potential limitation of the technique. To address
this, compilers can introduce various levels of heuristics, ranging from
applying the mitigations only to a small proportion of functions, to applying
it universally. See, for example, the `-fstack-protector`,
`-fstack-protector-strong` and `-fstack-protector-all` options offered by both
...
program's execution. An attacker that manages to obtain the canary value at
...
effects of overflows that happen through library functions is library function

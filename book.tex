...
% Replace csl-refs section from default pandoc template to make \citeproc work
% see https://github.com/Zettlr/Zettlr/issues/4879#issuecomment-1934165373
...
% start of llsoftsecbook-specific template part
\clearpage
...
Version 0-221-g283a066
\clearpage
% end of llsoftsecbook-specific template part
...
are usually defined (\citeproc{ref-Hicks2014}{Hicks 2014}) by explicitly
listing their types, which include:
...
\phantomsection\label{__index_entry_1}{C}\index{C}/\phantomsection\label{__index_entry_2}{C++}\index{C++}.
...
overflows appeared (\citeproc{ref-AlephOne1996}{Aleph One 1996}).
...
(\citeproc{ref-Solar1997}{Solar Designer 1997}). Code reuse attacks
later evolved to \hyperref[return-oriented-programming]{Return-Oriented
Programming (ROP)} (\citeproc{ref-Shacham2007}{Shacham 2007}) and even
...
Memory safe (\citeproc{ref-Hicks2014}{Hicks 2014}) languages are
...
On the one hand, \phantomsection\label{__index_entry_3}{C}\index{C} and
\phantomsection\label{__index_entry_4}{C++}\index{C++} remain very
popular languages, particularly in the implementation of low-level
software. On the other hand, programs written in memory safe languages
can themselves be vulnerable to memory errors as a result of bugs in how
they are implemented, e.g.~a bug in their compiler. Can we fix the
problem by also using memory safe languages for the compiler and runtime
...
\hyperref[jit-compiler-vulnerabilities]{later section}.
...
An \emph{\phantomsection\label{__index_entry_5}{exploit
primitive}\index{exploit primitive}} is a mechanism that allows an
...
A \emph{\phantomsection\label{__index_entry_6}{write
primitive}\index{write primitive}} gives the attacker some level of
write access to the victim's memory space. The value written and the
address written to may be controlled by the attacker to various degrees.
The primitive, for example, may allow:
...
properties. See slide 11 of (\citeproc{ref-Miller2012}{Miller, n.d.})
for an example.
...
A \emph{\phantomsection\label{__index_entry_7}{read
primitive}\index{read primitive}}, respectively, gives the attacker read
...
write primitive. A particularly useful primitive is an \emph{arbitrary
read} primitive, in which the address is fully controlled by the
attacker.
...
(\citeproc{ref-Hu2016}{Hu et al. 2016}).

\begin{itemize}
\tightlist
\item
  In an \emph{\phantomsection\label{__index_entry_8}{interactive
  attack}\index{interactive attack}}, the attacker gives malicious input
  to the victim program. The malicious input causes the victim program
  to perform the read the attacker instructed it to, and to output the
  results of that read. This output could be any kind of output, for
  example a network packet that the victim transmits. The attacker can
  observe the result of the read primitive by looking at this output,
  for example parsing this network packet. This process then repeats:
  the attacker sends more malicious input to the victim, observes the
  output and prepares the next input. You can see an example of this
  type of attack in (\citeproc{ref-Beer2020}{Beer 2020}), which
  describes a zero-click radio proximity exploit.
\item
  In a \emph{\phantomsection\label{__index_entry_9}{non-interactive
  (one-shot) attack}\index{non-interactive (one-shot) attack}}, the
  attacker provides all malicious input to the victim program at once.
  The malicious input triggers multiple primitives one after the other,
  ...
  example, in the form of a JavaScript program
  (\citeproc{ref-Grouxdf2020}{Groß 2020}), or a PDF file pretending to
  be a GIF (\citeproc{ref-Beer2021}{Beer and Groß 2021}).
\end{itemize}
...
explored further in (\citeproc{ref-Dullien2020}{Dullien 2020}).
...
isn't as obvious. For example, in (\citeproc{ref-Beer2021}{Beer and Groß
2021}), the malicious input consists of a PDF file, masquerading as a
...
goals is \phantomsection\label{__index_entry_10}{arbitrary code
execution}\index{arbitrary code execution} within the victim process. We
...
\item
  ...
  \hyperref[return-oriented-programming]{ROP attack}.
\item
...
\phantomsection\label{ex:stack-buffer-overflow}
\begin{example}
...
write
\phantomsection\label{__index_entry_11}{shellcode}\index{shellcode}\footnote{A
...
(\citeproc{ref-AlephOne1996}{Aleph One 1996}) is a classic example of
this technique.
...
on writing secure
\phantomsection\label{__index_entry_12}{C}\index{C}/\phantomsection\label{__index_entry_13}{C++}\index{C++}
code, for example (\citeproc{ref-Seacord2013}{Seacord 2013}) and
(\citeproc{ref-Dowd2006}{Dowd, McDonald, and Schuh 2006}). A different
...
like \texttt{\_FORTIFY\_SOURCE} (\citeproc{ref-Sharma2014}{Sharma
2014}).
...
A \emph{\phantomsection\label{__index_entry_14}{use after
free}\index{Use after free (UaF)}} (UaF) occurs when a variable is used
...
\phantomsection\label{__index_entry_15}{Common Weakness
Enumeration}\index{Common Weakness Enumeration (CWE)} (CWE) page
...
techniques, see (\citeproc{ref-dhavalkapil2022}{Kapil 2022}).

\phantomsection\label{ex:use-after-free}
\begin{example}
...
\tododiv{Dive more into UaF detection and mitigations. Suggested
starting points:\\
Allocators using different algorithms to reduce the probability of
exploiting use-after-free (this might be a whole section of it's
own?)Type-aware allocation and deallocation functions, which explains
the motivation for this feature at
https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p2719r5.html\#a-concrete-use-case.Languages
that use garbage collection or reference counting}

Those are not exaustive lists of tools to detect, mitigate and prevent
UaFs. But this section's goal was to give a brief introduction to the
topic.
...
simply place
\phantomsection\label{__index_entry_16}{shellcode}\index{shellcode} of
...
(\citeproc{ref-Solar1997}{Solar Designer 1997}). Instead of returning to
...
\textbf{\phantomsection\label{__index_entry_17}{gadgets}\index{gadgets}}.
...
In \phantomsection\label{__index_entry_18}{return-oriented programming
(ROP)}\index{return-oriented programming (ROP)}
(\citeproc{ref-Shacham2007}{Shacham 2007}), each gadget performs a
...
call stack (often called a \phantomsection\label{__index_entry_19}{ROP
chain}\index{ROP chain}) which ensures a number of gadgets are executed
...
Figure \hyperref[fig:rop-control-flow]{1} shows this control flow.
...
\hyperref[fig:rop-call-stack]{2}, where ``Original frame'' marks the
frame in which the address of \texttt{gadget\_x2} has replaced a saved
return address that will be loaded and returned to in the future. As an
...
stack pointer's value instead. This is known as
\phantomsection\label{__index_entry_20}{stack
pivoting}\index{stack pivoting}.

Note that this fake call stack contains zero bytes, even without
...
stack in one go, since C-style strings are
\href{https://en.wikipedia.org/wiki/Null-terminated_string}{null-terminated}
and copying the fake stack contents would stop once the first zero byte
is encountered. The ROP chain would therefore need to be adjusted so
that it doesn't contain zero bytes, for example by initially replacing
the zero bytes with a different byte and adding some more gadgets to the
ROP chain that write zero to those stack locations.
...
use ``\phantomsection\label{__index_entry_21}{gadget
scanner}\index{gadget scanner}'' tools in order to discover large
numbers of gadgets automatically. Such tools can also be useful to a
compiler engineer working on a code reuse attack mitigation, as they can
point out code sequences that should be protected and have been missed.
...
\phantomsection\label{__index_entry_22}{Jump-oriented programming
(JOP)}\index{Jump-oriented programming (JOP)}
(\citeproc{ref-Bletsch2011}{Bletsch et al. 2011}) is a variation on ROP,
where gadgets can also end in indirect branch instructions instead of
return instructions. The attacker chains a number of such gadgets
through a \phantomsection\label{__index_entry_23}{dispatcher
...
In figure \hyperref[fig:jop]{3}, \texttt{x4} initially points to the
``dispatch table'', which has been modified by the attacker to contain
the addresses of the three gadgets they want to execute. The dispatcher
...
\phantomsection\label{__index_entry_24}{Counterfeit Object-oriented
programming
(COOP)}\index{Counterfeit Object-oriented programming (COOP)}
(\citeproc{ref-Schuster2015}{Schuster et al. 2015}) is a code reuse
technique that takes advantage of
\phantomsection\label{__index_entry_25}{C++}\index{C++} virtual function
...
functions used as gadgets in the attack are called
\phantomsection\label{__index_entry_26}{vfgadgets}\index{vfgadgets}. To
chain vfgadgets together, the attacker uses a ``main loop gadget'',
similar to JOP's dispatcher gadget, which is itself a virtual function
that loops over a container of pointers to C++ objects and invokes a
virtual function on these objects. (\citeproc{ref-Schuster2015}{Schuster
et al. 2015}) describes the attack in more detail. It is specifically
mentioned here as an example of an attack that doesn't depend on
directly replacing return addresses and code pointers, like ROP and JOP
do. Such language-specific attacks are important to consider when
considering mitigations against code reuse attacks, which will be the
topic of the next section.

\tododiv{It would be nice to have a small example of a COOP attack,
similar to the JOP example in the previous section.
\href{https://github.com/llsoftsec/llsoftsecbook/issues/261}{\#261}}

\subsection{Sigreturn-oriented
programming}\label{sec:sigreturn-oriented-programming}

One last example of a code reuse attack that is worth mentioning here is
\phantomsection\label{__index_entry_27}{sigreturn-oriented programming
(SROP)}\index{sigreturn-oriented programming (SROP)}
(\citeproc{ref-Bosman2014}{Bosman and Bos 2014}). It is a special case
of ROP where the attacker creates a fake signal handler frame and calls
\texttt{sigreturn}. \texttt{sigreturn} is a system call on many
UNIX-type systems which is normally called upon return from a signal
handler, and restores the state of the process based on the state that
has been saved on the signal handler's stack by the kernel previously,
on entry to the signal handler. The ability to fake a signal handler
frame and call \texttt{sigreturn} gives an attacker a simple way to
...
\phantomsection\label{__index_entry_28}{registers}\index{register}.
Overwriting value in machine registers directly is often not possible.
Attackers use \phantomsection\label{__index_entry_29}{memory
vulnerabilities}\index{memory vulnerability} to be able to overwrite
...
\href{https://en.wikipedia.org/wiki/Address_space_layout_randomization}{\phantomsection\label{__index_entry_30}{Address
space layout randomization
(ASLR)}\index{Address space layout randomization (ASLR)}} makes this
...
often referred to as an ``\phantomsection\label{__index_entry_31}{info
leak}\index{info leak}'' (\citeproc{ref-Serna2012}{Serna 2012}).
...
\href{https://en.wikipedia.org/wiki/Control-flow_integrity}{\phantomsection\label{__index_entry_32}{Control-flow
integrity (CFI)}\index{Control-flow integrity (CFI)}} is a family of
...
to as \phantomsection\label{__index_entry_33}{forward-edge
CFI}\index{forward-edge CFI}, whereas a scheme that protects returns is
said to implement \phantomsection\label{__index_entry_34}{backward-edge
CFI}\index{backward-edge CFI}.
...
\phantomsection\label{__index_entry_35}{coarse-grained}\index{coarse-grained CFI}
or
\phantomsection\label{__index_entry_36}{fine-grained}\index{fine-grained CFI}.
...
transfer targets. (\citeproc{ref-Burow2017}{Burow et al. 2017}) compares
...
\item
  ...
  CFI}
\item
  arm64e, see McCall and Bougacha (\citeproc{ref-McCall2019}{2019}) and
  pauthabi, see Korobeynikov (\citeproc{ref-Korobeynikov2024}{2024})
\item
  \href{https://reviews.llvm.org/D119296}{kcfi}
\item
  various shadow stacks
\item
  pac-ret, see Cheeseman (\citeproc{ref-Cheeseman2019}{2019})
\item
...
\phantomsection\label{__index_entry_37}{equivalence
...
\phantomsection\label{__index_entry_38}{signature}\index{function signature}.

For example,
\phantomsection\label{__index_entry_39}{arm64e}\index{arm64e} and
\phantomsection\label{__index_entry_40}{pauthabi}\index{pauthabi} put
all C functions in a single equivalence class see McCall and Bougacha
(\citeproc{ref-McCall2019}{2019}). Examples of CFI schemes that
...
\phantomsection\label{ex:qsort-cfi}
\begin{example}
...
\phantomsection\label{__index_entry_41}{C++}\index{C++} virtual function
...
\href{https://en.wikipedia.org/wiki/Branch_table}{\phantomsection\label{__index_entry_42}{jump
table}\index{jump table}}, which is an array of pointers or offsets to
...
\phantomsection\label{__index_entry_43}{arm64e}\index{arm64e} and
\phantomsection\label{__index_entry_44}{pauthabi}\index{pauthabi} do, as
explained in the example below. This is also explained in
(\citeproc{ref-McCall2019}{McCall and Bougacha 2019}, slide 39-40).
...
``\phantomsection\label{__index_entry_45}{return
address}\index{return address}''. When the called function returns, it
...
\phantomsection\label{__index_entry_46}{Shadow
stack}\index{shadow stack} approaches store the return address on a
second stack. Some shadow stack approaches also store the return address
in the original location in the normal stack. In those, before the
return is executed, it verifies that the return value on both the
...
\paragraph{Other code pointers that may need
protection}\label{other-code-pointers-that-may-need-protection}

Anytime a code pointer is stored in memory, it can potentially be
modified by an attacker with a write primitive. The previous sections
gave examples of how code pointers may originate from various source
code constructs, such as function pointers, vtables, return addresses,
etc. This list isn't exhaustive, and there are more source code
constructs that can lead to code pointers being stored in memory, such
as:

\begin{itemize}
\tightlist
\item
  C++ co-routines are typically implemented using structures containing
  code pointers. Abusing these has recently been coined as
  \href{https://www.usenix.org/conference/usenixsecurity25/presentation/bajo}{\phantomsection\label{__index_entry_47}{Coroutine
  Frame-Oriented Programming
  (CFOP)}\index{Coroutine Frame-Oriented Programming (CFOP)}}.
\item
  \phantomsection\label{__index_entry_48}{Procedure Linkage Table
  (PLT)}\index{Procedure Linkage Table (PLT)} and the
  \href{https://en.wikipedia.org/wiki/Global_Offset_Table}{\phantomsection\label{__index_entry_49}{Global
  Offset Table(GOT)}\index{Global Offset Table(GOT)}} often contain code
  pointers. One common way to protect these from being overwritten by an
  attacker is to make these tables
  \href{https://www.redhat.com/en/blog/hardening-elf-binaries-using-relocation-read-only-relro}{read-only
  during program startup}.
\item
  Signal handlers and signal handler frames contain code pointers, see
  \hyperref[sec:sigreturn-oriented-programming]{2.5.4} for more details.
  \todospan{{Should we list examples of indirect control flow from other
  languages too?}}
\end{itemize}

\subsubsection{Detailed descriptions of a few CFI
schemes}\label{detailed-descriptions-of-a-few-cfi-schemes}
...
\paragraph{Clang Shadow Stack}\label{sec:clang-shadow-stack}

Clang also implements a backward-edge CFI scheme known as
\href{https://clang.llvm.org/docs/ShadowCallStack.html}{\phantomsection\label{__index_entry_50}{Shadow
...
\hyperref[stack-buffer-overflow]{earlier stack buffer overflow example}
...
\paragraph{Pointer Authentication}\label{sec:pointer-authentication}
...
One such example is \phantomsection\label{__index_entry_51}{Pointer
Authentication}\index{Pointer Authentication}
(\citeproc{ref-Rutland2017}{Rutland 2017}), an Armv8.3 feature,
...
address, called a \phantomsection\label{__index_entry_52}{Pointer
Authentication Code (PAC)}\index{Pointer Authentication Code (PAC)}, see
...
\emph{\phantomsection\label{__index_entry_53}{signed
pointer}\index{signed pointer}}. A non-signed pointer is called a
\emph{\phantomsection\label{__index_entry_54}{raw
pointer}\index{raw pointer}}.
...
\phantomsection\label{__index_entry_55}{key}\index{key} and a
\phantomsection\label{__index_entry_56}{modifier}\index{modifier}:
...
\item
  ...
  called a \phantomsection\label{__index_entry_57}{pointer substitution
  attack}\index{pointer substitution attack}. Using different modifiers
  for different contexts makes pointer substitution attacks harder.
\end{itemize}

\begin{figure}
\centering
\includegraphics[width=1\textwidth,height=\textheight]{img/pauth_sign_auth.pdf}
...
(\citeproc{ref-Liljestrand2021}{Liljestrand et al. 2021}), Camouflage
(\citeproc{ref-DenisCourmont2021}{Denis-Courmont et al. 2021}), PAL
(\citeproc{ref-Yoo2021}{Yoo et al. 2021}), PTAuth
(\citeproc{ref-farkhani2021}{farkhani, Ahmadi, and Lu 2021}), PAC it up
(\citeproc{ref-Liljestrand2019}{Liljestrand et al. 2019}), FIPAC
(\citeproc{ref-Schilling2022}{Schilling, Nasahl, and Mangard 2022}),
...
\phantomsection\label{ex:pac-ret}
\begin{example}
...
After the \texttt{paciasp} instruction, the value in \texttt{x30} is a
signed pointer. The \texttt{stp} instruction stores the signed pointer
to memory. Under the usual threat model, an attacker with a write
primitive can modify the value while it is in memory. Therefore, after
the value is loaded into \texttt{x30} again, by the \texttt{ldp}
instruction, it should be considerded to be potentially tampered with.

Therefore, the compiler inserts the \texttt{autiasp} instruction between
loading the signed pointer from memory and using it in the \texttt{ret}
instruction. The \texttt{autiasp} instruction verifies the PAC in the
upper bits of \texttt{x30}, taking into account the secret key
\texttt{IA} and modifier \texttt{sp}. If the PAC is correct, which will
be the case in normal execution, the extension bits of the address are
restored, so that the address can be used in the \texttt{ret}
instruction. However, if the PAC is incorrect, the upper bits will be
corrupted so that subsequent uses of the address (such as in the
\texttt{ret} instruction) will result in a fault.
...
The probability of success when guessing a PAC depends on the exact
number of PAC bits available in a given system configuration.

The authenticated pointers are vulnerable to
\phantomsection\label{__index_entry_58}{pointer substitution
attacks}\index{pointer substitution attacks}, where a pointer that has
been signed with a given modifier is replaced with a different pointer
that has also been signed with the same modifier. In the
\texttt{pac-ret} scheme, this is mitigated by using the stack pointer as
the modifier, which limits reuse of signed return address pointers to
function frames that happen to have the same stack pointer value.

\subparagraph{arm64e and pauthabi: Forward-Edge
CFI}\label{sec:arm64e-pauthabi}

\tododiv{The use of pauth in arm64e or pauthabi should be explained in
more detail, including the concepts of signing and authentication
oracles
\href{https://github.com/llsoftsec/llsoftsecbook/issues/259}{\#259}}
...
(\citeproc{ref-McCall2019}{McCall and Bougacha 2019}). The Pointer
...
\href{https://developer.arm.com/documentation/102433/0100/Jump-oriented-programming?lang=en}{\phantomsection\label{__index_entry_59}{Branch
Target Identification (BTI)}\index{Branch Target Identification (BTI)}},
...
(\citeproc{ref-Conti2015}{Conti et al. 2015}) describes how CFI
...
overwriting \phantomsection\label{__index_entry_60}{control
data}\index{control data}, which are used to change the value of the
...
\phantomsection\label{__index_entry_61}{Non-control data
attacks}\index{non-control data attacks}, also known as
\phantomsection\label{__index_entry_62}{data-only
attacks}\index{data-only attacks}, can completely bypass these
...
expressiveness (\citeproc{ref-Beer2021}{Beer and Groß 2021}). A very
...
real applications, see (\citeproc{ref-Chen2005}{Chen et al. 2005}).
...
(\citeproc{ref-Hu2016}{Hu et al. 2016}) that they can, in fact, be very
expressive. (\citeproc{ref-Hu2016}{Hu et al. 2016}) describes
...
in the program\footnote{The authors describe how DOP gadgets can be
  ...
  explained in (\citeproc{ref-Dullien2018}{Flake 2018}). Many
...
The authors of (\citeproc{ref-Hu2016}{Hu et al. 2016}) describe a small
...
automated. Related literature (\citeproc{ref-Ispoglou2018}{Ispoglou et
al. 2018}) focuses on automating data-only attacks.
...
to
\phantomsection\label{__index_entry_63}{C}\index{C}/\phantomsection\label{__index_entry_64}{C++}\index{C++}
programmers to help them detect vulnerabilities that can lead to memory
errors.

\subsection{Sanitizers}\label{sanitizers}

\phantomsection\label{__index_entry_65}{Sanitizers}\index{sanitizers}
...
\phantomsection\label{__index_entry_66}{fuzzers}\index{fuzzing}\footnote{\href{https://en.wikipedia.org/wiki/Fuzzing}{\phantomsection\label{__index_entry_67}{Fuzzing}\index{fuzzing}}
...
\href{https://clang.llvm.org/docs/AddressSanitizer.html}{\phantomsection\label{__index_entry_68}{Address
Sanitizer (ASan)}\index{Address Sanitizer (ASan)}}. It aims to detect
...
use-after-free, double-free and invalid free\footnote{ASan also includes
  a
  \href{https://clang.llvm.org/docs/LeakSanitizer.html}{\phantomsection\label{__index_entry_69}{memory
  leak detector}\index{LeakSanitizer}}.}. There are Address Sanitizer
implementations for both
\phantomsection\label{__index_entry_70}{GCC}\index{GCC} and
\phantomsection\label{__index_entry_71}{Clang}\index{Clang}, but we will
focus on the Clang implementation here.

ASan uses \phantomsection\label{__index_entry_72}{shadow
memory}\index{shadow memory} to keep track of the state of the
...
(\citeproc{ref-Serebryany2012}{Serebryany et al. 2012}). Shadow memory
...
\phantomsection\label{__index_entry_73}{\texttt{malloc}}\index{malloc}
and \phantomsection\label{__index_entry_74}{\texttt{free}}\index{free}
with its own specialized versions. \texttt{malloc} introduces
\phantomsection\label{__index_entry_75}{redzones}\index{redzone} before
and after each allocation, which are marked as unaddressable.
...
the Linux kernel,
\href{https://www.kernel.org/doc/html/v5.0/dev-tools/kasan.html}{\phantomsection\label{__index_entry_76}{KASAN}\index{KASAN}}.

ASan's biggest drawback is its high runtime overhead and memory usage,
due to the quarantine, redzones and shadow memory.
\href{https://clang.llvm.org/docs/HardwareAssistedAddressSanitizerDesign.html}{\phantomsection\label{__index_entry_77}{Hardware-assisted
AddressSanitizer
(HWASAN)}\index{Hardware-assisted AddressSanitizer (HWASAN)}} works
similarly to ASan, but with partial hardware assistance can result in
lower memory overheads, at the cost of being less portable.

On \phantomsection\label{__index_entry_78}{AArch64}\index{AArch64},
HWASAN uses \phantomsection\label{__index_entry_79}{Top-Byte Ignore
(TBI)}\index{Top-Byte Ignore (TBI)}. When TBI is enabled, the top byte
of a pointer is ignored when performing a memory access, allowing
software to use that top byte to store metadata, without affecting
execution. Each allocation is aligned to 16 bytes and each 16-byte chunk
of memory (called ``granule'') is randomly assigned an 8-bit tag. The
tag is stored in shadow memory and is also placed in the top byte of the
pointer to the object. Memory loads and stores are then instrumented to
check that the tag stored in the pointer matches the tag stored in
memory, and report an error when a mismatch happens. \todospan{{Add
diagram to demonstrate how HWASAN works
...
\href{https://llvm.org/docs/MemTagSanitizer.html}{\phantomsection\label{__index_entry_80}{MemTagSanitizer}\index{MemTagSanitizer}}
goes one step further and uses the Armv8.5-A
\href{https://developer.arm.com/documentation/102925/0100}{\phantomsection\label{__index_entry_81}{Memory
Tagging Extension (MTE)}\index{Memory Tagging Extension (MTE)}}. With
MTE, the tag checking is done automatically by hardware, and an
exception is raised on mismatch. MTE's granule size is 16 bits, whereas
tags are 4-bit. \todospan{{Consider adding a whole section on MTE and
its applications
\href{https://github.com/llsoftsec/llsoftsecbook/issues/169}{\#169}}}

\href{https://clang.llvm.org/docs/UndefinedBehaviorSanitizer.html\#ubsan-checks}{\phantomsection\label{__index_entry_82}{UndefinedBehaviorSanitizer
(UBSan)}\index{UndefinedBehaviorSanitizer (UBSan)}} detects undefined
behavior during program execution, for example array out-of-bounds
accesses for statically determined array bounds, null pointer
dereference, signed integer overflow and various kinds of integer
conversions that result in data loss. Although some of these checks are
not directly related to memory errors, these kinds of errors can lead to
incorrect pointer arithmetic, incorrect allocation sizes, and other
issues that lead to memory errors, so it is important to detect them and
address them.
...
\tightlist
\item
  \href{https://clang.llvm.org/docs/MemorySanitizer.html}{\phantomsection\label{__index_entry_83}{MemorySanitizer}\index{MemorySanitizer}}:
  detects uninitialized reads.
\item
  \href{https://clang.llvm.org/docs/ThreadSanitizer.html}{\phantomsection\label{__index_entry_84}{ThreadSanitizer}\index{ThreadSanitizer}}:
  detects data races.
\item
  \href{https://llvm.org/docs/GwpAsan.html}{\phantomsection\label{__index_entry_85}{GWP-ASan}\index{GWP-ASan}}:
...
usually described with the term
``\phantomsection\label{__index_entry_86}{spatial memory
safety}\index{spatial memory safety}''. Out-of-bounds accesses result in
restricted \phantomsection\label{__index_entry_87}{read/write
primitives}\index{read primitive}\phantomsection\label{__index_entry_88}{}\index{write primitive}\footnote{These
  primitives are restricted since they can only access a limited number
  of bytes past the end of the allocation.}. An attacker can often
easily convert these into arbitrary read/write primitives. For example,
this can be achieved by overwriting pointer fields in allocations
following the object that was the target of the problematic memory
access.
...
catch \phantomsection\label{__index_entry_89}{intra-object
overflows}\index{intra-object overflow} on array member accesses that
...
annotations at
\href{https://en.wikipedia.org/wiki/Application_binary_interface}{\phantomsection\label{__index_entry_90}{Application
Binary Interface (ABI)}\index{Application Binary Interface (ABI)}}
...
in (\citeproc{ref-Cook2023}{Cook 2023}).
...
introduces
\emph{\phantomsection\label{__index_entry_91}{capabilities}\index{capability}}
...
\phantomsection\label{__index_entry_92}{\href{https://en.wikipedia.org/wiki/Just-in-time_compilation}{just-in-time
(JIT)} compilers}\index{just-in-time (JIT) compilers}\footnote{JIT
...
(\citeproc{ref-Pizlo2020}{Pizlo 2020}), recording the types encountered
...
\phantomsection\label{__index_entry_93}{deoptimization}\index{deoptimization}
or \phantomsection\label{__index_entry_94}{on-stack replacement
(OSR)}\index{on-stack replacement (OSR)}. Deoptimization makes sure that
...
(\citeproc{ref-Wingo2011}{Wingo 2011}). In short, JavaScript engines use
...
\phantomsection\label{__index_entry_95}{range
analysis}\index{range analysis} to determine as precise a range for a
value as possible, which is useful for
\phantomsection\label{__index_entry_96}{bounds check
elimination}\index{bounds check elimination}.

\phantomsection\label{__index_entry_97}{Bounds check elimination
(BCE)}\index{Bounds check elimination (BCE)} is a common optimization in
...
(\citeproc{ref-Glazunov2021}{Glazunov 2021}) lists a few examples of
...
encountered in a JavaScript engine. (\citeproc{ref-Grouxdf2022}{Groß and
Burnett 2022}) lists some other common types of bugs that result in
...
\item
  ...
  of bounds checks against typer bugs} \footnote{This naturally leads to
    ...
    (\citeproc{ref-Fetiveau2019}{Fetiveau 2019}).}.
\item
  ...
  built on top of
  \href{https://v8.dev/blog/pointer-compression}{\phantomsection\label{__index_entry_98}{pointer
...
\item
  ...
  \href{https://en.wikipedia.org/wiki/W\%5EX}{\phantomsection\label{__index_entry_99}{W\^{}X}\index{W\textasciicircum{}X}}.
  ...
  attackers from writing to code memory (\citeproc{ref-Song2015}{Song et
...
reader could take a look at (\citeproc{ref-saelo2021a}{saelo 2021b}) and
(\citeproc{ref-saelo2021b}{saelo 2021a}).
...
A \textbf{\phantomsection\label{__index_entry_100}{covert
channel}\index{covert channel}} is a channel where both entities intend
to communicate. A
\textbf{\phantomsection\label{__index_entry_101}{side-channel}\index{side-channel}}
...
\textbf{\phantomsection\label{__index_entry_102}{victim}\index{victim}}.
The other entity is sometimes called the
\textbf{\phantomsection\label{__index_entry_103}{spy}\index{spy}}.
...
\phantomsection\label{__index_entry_104}{micro-architectural}\index{micro-architectural}
...
\phantomsection\label{__index_entry_105}{timing
attacks}\index{timing attacks}.
...
property (a) is an active research area. (\citeproc{ref-Wu2018}{Wu et
al. 2018}) provides a method to convert a program such that it respects
property (a), albeit by potentially introducing unsafe memory accesses.
(\citeproc{ref-Soares2021}{Soares and Pereira 2021}) improves on that
result by not introducing unsafe memory accesses, albeit by potentially
needing to change the interface of the transformed
...
found in (\citeproc{ref-Pornin2018}{Pornin 2018}).

As discussed in (\citeproc{ref-Pornin2018}{Pornin 2018}), when
...
\href{https://en.wikipedia.org/wiki/Cache_(computing)}{\phantomsection\label{__index_entry_106}{Caches}\index{cache}}
...
\phantomsection\label{__index_entry_107}{access
time}\index{memory access time} is 3 to 5 CPU cycles for the fastest
...
\textbf{\phantomsection\label{__index_entry_108}{cache
hit}\index{cache!hit}}. Otherwise, it's called a
\textbf{\phantomsection\label{__index_entry_109}{cache
miss}\index{cache!miss}}.

Most systems have \phantomsection\label{__index_entry_110}{multiple
levels of cache}\index{cache!multi-level}, each with a different
trade-off between \phantomsection\label{__index_entry_111}{cache
size}\index{cache!size} and
\phantomsection\label{__index_entry_112}{access
...
\href{https://en.wikipedia.org/wiki/Locality_of_reference}{\phantomsection\label{__index_entry_113}{principle
of
locality}\index{principle of locality}}{]}\phantomsection\label{__index_entry_114}{}\index{locality of reference}.

Data is stored and transferred between cache levels in blocks of aligned
memory. Such a block is called a
\textbf{\phantomsection\label{__index_entry_115}{cache
block}\index{cache!block}} or
\textbf{\phantomsection\label{__index_entry_116}{cache
line}\index{cache!line}}. Typical sizes are 32, 64 or 128 bytes per
cache line.

When data that wasn't previously in the cache needs to be stored in the
cache, room has to be made for it by removing, or
\textbf{\phantomsection\label{__index_entry_117}{evicting}\index{cache!eviction}},
some other address/data from it. How that choice gets made is decided by
the
\href{https://en.wikipedia.org/wiki/Cache_replacement_policies}{\phantomsection\label{__index_entry_118}{cache
replacement policy}\index{cache!replacement policy}}{]}. Popular
replacement algorithms are
\phantomsection\label{__index_entry_119}{Least Recently Used
(LRU)}\index{cache!LRU},
\phantomsection\label{__index_entry_120}{Random}\index{cache!random replacement policy}
and
\phantomsection\label{__index_entry_121}{pseudo-LRU}\index{cache!pseudo-LRU replacement policy}.
...
the cache is
\textbf{\phantomsection\label{__index_entry_122}{fully-associative}\index{cache!fully-associative}}.
Most caches are however not fully-associative, as it's too costly to
implement. Instead, most caches are
\textbf{\phantomsection\label{__index_entry_123}{set-associative}\index{cache!set-associative}}.
In an N-way set-associative cache, a specific line can only be stored in
one of N cache locations. For example, if a line can potentially be
stored in one of 2 locations, the cache is said to be 2-way
set-associative. If it can be stored in one of 4 locations, it's called
4-way set-associative, and so on. When an address can only be stored in
one location in the cache, it is said to be
\textbf{\phantomsection\label{__index_entry_124}{direct-mapped}\index{cache!direct-mapped}},
...
is called a \textbf{\phantomsection\label{__index_entry_125}{cache
set}\index{cache!set}}.
...
\caption{Figure 6: Illustration of indexing into a set-associative
...
\hyperref[fig:cache-indexing]{6}. The least-significant \(L\) bits,
where \(2^L\) is the cache line size, are used to compute an address's
offset within a cache line. The next \(S\) bits, where \(2^S\) is the
number of cache sets, are used to determine which cache set an address
maps to. The remaining top bits are ``tag bits''. They are stored
alongside a line in the cache so later operations can detect which
specific memory address is replicated in that cache line.
...
\tododiv{Also explain cache coherency cache coherency?
...
\phantomsection\label{__index_entry_126}{Cache
side-channels}\index{cache!side-channel} typically work by the spy
determining whether a memory access was a cache hit or a cache miss.
From that information, the spy may be able to deduce bits of data that
only the victim should have access to.
...
In a so-called
\textbf{\phantomsection\label{__index_entry_127}{Flush+Reload}\index{Flush+Reload}}
attack(\citeproc{ref-Yarom2014}{Yarom and Falkner 2014}), the spy
process shares memory with the victim process. The attack works in 3
steps:
...
(\citeproc{ref-Yarom2014}{Yarom and Falkner 2014}) demonstrates that a
Flush+Reload attack can be used to leak GnuPG private keys.

\subsubsection{Prime+Probe}\label{primeprobe}

In a
\textbf{\phantomsection\label{__index_entry_128}{Prime+Probe}\index{Prime+Probe}}
attack, there is no need for memory to be shared between victim and spy.
The attack works in 3 steps:
...
(\citeproc{ref-Osvik2005}{Osvik, Shamir, and Tromer 2005}) first
documented this technique in 2005 and demonstrates extracting AES keys
...
follow a similar 3-step pattern. (\citeproc{ref-Weber2021}{Weber et al.
2021}) describes this general pattern and uses it to automatically
discover more side-channels that follow this 3-step pattern. They
describe the general pattern as being:
...
\item
  ...
  (\textbf{\phantomsection\label{__index_entry_129}{reset
  sequence}\index{reset sequence}}).
\item
  An instruction sequence that triggers a state change
  (\textbf{\phantomsection\label{__index_entry_130}{trigger
  sequence}\index{trigger sequence}}).
\item
  An instruction sequence that leaks the inner state
  (\textbf{\phantomsection\label{__index_entry_131}{measurement
  sequence}\index{measurement sequence}}).
\end{enumerate}

Other cache-based side channel attacks following this general 3-step
approach include:
\phantomsection\label{__index_entry_132}{Flush+Flush}\index{Flush+Flush}(\citeproc{ref-Gruss2016a}{Gruss,
Maurice, Wagner, et al. 2016}),
\phantomsection\label{__index_entry_133}{Flush+Prefetch}\index{Flush+Prefetch}(\citeproc{ref-Gruss2016}{Gruss,
Maurice, Fogh, et al. 2016}),
\phantomsection\label{__index_entry_134}{Evict+Reload}\index{Evict+Reload}(\citeproc{ref-Percival2005}{Percival
2005}),
\phantomsection\label{__index_entry_135}{Evict+Time}\index{Evict+Time}(\citeproc{ref-Osvik2005}{Osvik,
Shamir, and Tromer 2005}),
\phantomsection\label{__index_entry_136}{Reload+Refresh}\index{Reload+Refresh}(\citeproc{ref-Briongos2020}{Briongos
et al. 2020}),
\phantomsection\label{__index_entry_137}{Collide+Probe}\index{Collide+Probe}(\citeproc{ref-Lipp2020}{Lipp
et al. 2020}), etc.

\subsection{Mitigating cache side-channel
attacks}\label{mitigating-cache-side-channel-attacks}

As described in (\citeproc{ref-Su2021}{Su and Zeng 2021}), 3 conditions
...
(\citeproc{ref-Mushtaq2020}{Mushtaq et al. 2020}) and
(\citeproc{ref-Su2021}{Su and Zeng 2021}) summarize dozens of proposals
and implementations -- too many to try to describe them all here.

One popular such mitigation is disabling
\href{https://en.wikipedia.org/wiki/Multithreading_(computer_architecture)}{\phantomsection\label{__index_entry_138}{cpu
multithreading}\index{cpu multithreading}}. For example,
...
\href{https://developer.chrome.com/blog/site-isolation/}{\phantomsection\label{__index_entry_139}{site
isolation}\index{site isolation}} as implemented in many web browsers is
...
method}\phantomsection\label{__index_entry_140}{}\index{Spectre}.
...
\phantomsection\label{__index_entry_141}{NetSpectre}\index{NetSpectre}
attack (\citeproc{ref-Schwarz2019}{Schwarz et al. 2019}) where the
...
Furthermore, (\citeproc{ref-Schwarz2017}{Schwarz et al. 2017})
...
\href{https://en.wikipedia.org/wiki/Instruction_pipelining}{\phantomsection\label{__index_entry_142}{instruction
pipelines}\index{instruction pipeline}}.
\phantomsection\label{__index_entry_143}{}\index{pipeline} In an
...
the next instruction leads to a big performance loss.\footnote{Over
  ...
  flight. (\citeproc{ref-Eyerman2009}{Eyerman et al. 2009, sec. 4.2.3})
...
\phantomsection\label{__index_entry_144}{predict}\index{predict} which
...
\href{https://en.wikipedia.org/wiki/Branch_predictor}{\phantomsection\label{__index_entry_145}{branch
predictors}\index{branch predictor}}, such as:

\begin{itemize}
\tightlist
\item
  A predictor of the outcome of a
  \phantomsection\label{__index_entry_146}{conditional
  branch}\index{conditional branch direction predictor}:
  \phantomsection\label{__index_entry_147}{taken}\index{taken branch} or
  not taken. The prediction is typically
  \phantomsection\label{__index_entry_148}{history-based}\index{history-based prediction},
  i.e.~based on the outcome of this and other branches in the recent
  past.
\item
  A predictor of the \phantomsection\label{__index_entry_149}{target of
  a taken branch}\index{branch target predictor}, i.e.~the address of
  the next instruction after a taken branch.
\item
  A predictor that is specialized to
  \phantomsection\label{__index_entry_150}{predict the next instruction
  after a function return instruction}\index{return address predictor}.
\end{itemize}
...
\phantomsection\label{__index_entry_151}{}\index{conditional branch direction predictor}

Two examples are
\phantomsection\label{__index_entry_152}{BranchScope}\index{BranchScope}
(\citeproc{ref-Evtyushkin2018}{Evtyushkin et al. 2018}) and
\phantomsection\label{__index_entry_153}{BlueThunder}\index{BlueThunder}
(\citeproc{ref-Huo2019}{Huo et al. 2019}). These attacks infer whether a
...
\hyperref[aslr]{\phantomsection\label{__index_entry_154}{ASLR}\index{ASLR}}.

\subsubsection{Branch target predictor side-channel
attacks}\label{branch-target-predictor-side-channel-attacks}

\phantomsection\label{__index_entry_155}{}\index{Branch target predictor}

Two examples are
\phantomsection\label{__index_entry_156}{SBPA}\index{SBPA}
(\citeproc{ref-Aciicmez2007}{Aciiçmez, Kaya Koç, and Seifert 2007}) and
\phantomsection\label{__index_entry_157}{BranchShadow}\index{BranchShadow}
(\citeproc{ref-Lee2017}{Lee et al. 2017}). These earlier attacks are
based on making a branch in the spy process alias in the
\phantomsection\label{__index_entry_158}{Branch Target Buffer
(BTB)}\index{Branch Target Buffer (BTB)} with a targeted branch in the
victim process. They use methods such as timing difference,
\phantomsection\label{__index_entry_159}{last branch
records}\index{last branch record},
\phantomsection\label{__index_entry_160}{instruction
traces}\index{instruction trace} or
\phantomsection\label{__index_entry_161}{performance
counters}\index{performance counter} to measure whether the branch in
...
\phantomsection\label{__index_entry_162}{}\index{return address predictor}

One example is
\phantomsection\label{__index_entry_163}{Hyper-Channel}\index{Hyper-Channel}
(\citeproc{ref-Bulygin2008}{Bulygin 2008}). In this case, a spy process
...
similar to \hyperref[general-schema-for-cache-covert-channels]{cache
side channels}:
...
\item
  ...
  (\emph{\phantomsection\label{__index_entry_164}{reset
  sequence}\index{reset sequence}}), run by the spy process.
\item
  An instruction sequence that triggers a branch predictor state change
  (\emph{\phantomsection\label{__index_entry_165}{trigger
  sequence}\index{trigger sequence}}), run by the victim process.
\item
  An instruction sequence that leaks the branch predictor state
  (\emph{\phantomsection\label{__index_entry_166}{measurement
  sequence}\index{measurement sequence}}), run by the spy process.
\end{enumerate}
...
\section{Transient execution attacks}\label{transient-execution-attacks}
...
\textbf{\phantomsection\label{__index_entry_167}{control
dependencies}\index{control dependency}}, where whether the later
instruction should be executed at all is dependent on the outcome of the
earlier instruction. Other kinds are
\textbf{\phantomsection\label{__index_entry_168}{true data
dependencies}\index{true data dependency}},
\textbf{\phantomsection\label{__index_entry_169}{anti-dependencies}\index{data dependency}}
and \textbf{\phantomsection\label{__index_entry_170}{output
dependencies}\index{output dependency}}. More details about these kinds
...
\textbf{\phantomsection\label{__index_entry_171}{predictions}\index{prediction}}.
...
\textbf{\phantomsection\label{__index_entry_172}{speculation}\index{speculation}}.
...
\textbf{\phantomsection\label{__index_entry_173}{mis-speculation}\index{mis-speculation}},
are called \textbf{\phantomsection\label{__index_entry_174}{transient
instructions}\index{transient instructions}}.\footnote{Transient
  ...
  been called
  \textbf{\phantomsection\label{__index_entry_175}{wrong-path
  instructions}\index{wrong-path instructions}} Mutlu et al.
  (\citeproc{ref-Mutlu2004}{2004})}
...
\textbf{\phantomsection\label{__index_entry_176}{architectural
effects}\index{architectural effects}} of transient instructions need to
be undone, but the
\textbf{\phantomsection\label{__index_entry_177}{micro-architectural
effects}\index{micro-architectural effects}} do not need to be undone.
...
\textbf{\phantomsection\label{__index_entry_178}{Faulting
instructions}\index{faulting instructions}} are instructions that
...
\textbf{\phantomsection\label{__index_entry_179}{transient
...
\textbf{\phantomsection\label{__index_entry_180}{Transient execution
attacks}\index{transient execution attacks}} are a category of
side-channel attacks that use the micro-architectural side-effects of
transient execution as a side channel.

The publication of the
\phantomsection\label{__index_entry_181}{Spectre}\index{Spectre}
(\citeproc{ref-Kocher2019}{Kocher et al. 2019}) and
\phantomsection\label{__index_entry_182}{Meltdown}\index{Meltdown}
(\citeproc{ref-Lipp2018}{Lipp et al. 2018}) attacks in 2018 started a
...
(\citeproc{ref-Bulck2020}{Bulck et al., n.d.}). There are alternative
categorizations. (\citeproc{ref-Bulck2020}{Bulck et al., n.d.}) defines
4 big classes of transient side-channel attack categories, based on
whether:
...
\end{minipage}} &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.7083} + 2\tabcolsep}@{}}{%
\begin{minipage}[b]{\linewidth}\raggedright
...
\hyperref[side-channels-through-branch-predictors]{Side-channels through
branch predictors}.
...
(\citeproc{ref-Karger1974}{Paul A. and Roger R. 1974}). In this attack,
...
Trusting Trust} (\citeproc{ref-Thompson1984}{Thompson 1984}).
...
Delphi toolchain with malicious code (\citeproc{ref-Gostev2009}{Gostev
2009}). When Delphi compiles a project the malicious library is included
...
infected thousands of iOS applications (\citeproc{ref-Cox2015}{Cox
2015}). The source of the infection was tracked down to a trojanized
...
(\citeproc{ref-Greenberg2019}{Greenberg 2019}). Some instances of the
...
Thompson (\citeproc{ref-Thompson1984}{Thompson 1984}) provides one of
the oldest and most popular examples in this area. In his paper, he
talks about a compiler that can detect when it is compiling the login
...
by various works such as (\citeproc{ref-wang2012undefined}{Wang et al.
2012}) (\citeproc{ref-d2015correctness}{D'Silva, Payer, and Song 2015})
(\citeproc{ref-dusilent}{Du, Wu, and Mao, n.d.}). By reading the works
...
random number generator (RNG) in Mac OS X (\citeproc{ref-Wang2015}{Wang
2015}). At some point in the past, this vulnerability affected all *BSD
...
(\citeproc{ref-FbsdJunk}{{``FreeBSD Mitigation for Srandom Undefined
Behavior''} 2012}) and OpenBSD (\citeproc{ref-ObsdJunk}{{``OpenBSD
Mitigation for Srandom Undefined Behavior''} 2002}).
...
(\citeproc{ref-ChromiumIssue}{{``Issue 3782: V8 Is Not -Fsanitize=null
Clean''} 2014}) and Mozilla (\citeproc{ref-MozillaIssue}{{``GCC6 - TB
Crashes Due to Removed Null Pointer Checks for "This"''} 2016}) had
...
\phantomsection\label{__index_entry_183}{dead store optimization
(DSO)}\index{dead store optimization (DSO)}. This has serious security
...
It still uses
\phantomsection\label{__index_entry_184}{\texttt{memset}}\index{memset}
...
(\citeproc{ref-MemZeroBarrier}{{``Lib: Memzero\_explicit: Use Barrier
Instead of {OPTIMIZER\_HIDE\_VAR}''} 2015}). In case of using LTO, the
...
(\citeproc{ref-MemZeroDataBarrier}{{``Lib: Make Memzero\_explicit More
Robust Against Dead Store Elimination''} 2015}). If the content of the
...
(\citeproc{ref-OpenSSLMemClr}{{``Lib: Make Memzero\_explicit More Robust
Against Dead Store Elimination''} 2016}). The approach OpenSSL used is
...
(\citeproc{ref-MemSetProposal}{{``Memset\_explicit''} 2021}). This
...
GNU presents in (\citeproc{ref-GNUMemSet}{{``10.604 Memset\_explicit,''}
n.d.}). The information may be present somewhere in the machine, even if
...
(\citeproc{ref-Wheeler2020}{Wheeler 2020}) provides a more systematic
...
added to the language (\citeproc{ref-pep572}{Angelico, Peters, and
Rossum 2018}) (in python 3.8) the more visually distinct ``walrus
...
(\citeproc{ref-Boucher2023}{Boucher and Anderson 2023}) is another way
...
\bibitem[\citeproctext]{ref-Beer2021}
...
\bibitem[\citeproctext]{ref-Bletsch2011}
...
\bibitem[\citeproctext]{ref-Bosman2014}
...
\bibitem[\citeproctext]{ref-Boucher2023}
...
\bibitem[\citeproctext]{ref-Briongos2020}
...
\bibitem[\citeproctext]{ref-Bulck2020}
...
\bibitem[\citeproctext]{ref-Bulygin2008}
...
\bibitem[\citeproctext]{ref-Burow2017}
...
\bibitem[\citeproctext]{ref-Chen2005}
...
\bibitem[\citeproctext]{ref-Conti2015}
...
\bibitem[\citeproctext]{ref-d2015correctness}
...
\bibitem[\citeproctext]{ref-DenisCourmont2021}
...
\bibitem[\citeproctext]{ref-Dowd2006}
...
\bibitem[\citeproctext]{ref-dusilent}
...
\bibitem[\citeproctext]{ref-Dullien2020}
...
\bibitem[\citeproctext]{ref-Evtyushkin2018}
...
\bibitem[\citeproctext]{ref-Eyerman2009}
...
\bibitem[\citeproctext]{ref-farkhani2021}
...
\bibitem[\citeproctext]{ref-Fetiveau2019}
...
\bibitem[\citeproctext]{ref-Dullien2018}
...
\bibitem[\citeproctext]{ref-FbsdJunk}
...
\bibitem[\citeproctext]{ref-MozillaIssue}
...
\bibitem[\citeproctext]{ref-Glazunov2021}
...
\bibitem[\citeproctext]{ref-Gruss2016}
...
\bibitem[\citeproctext]{ref-Gruss2016a}
...
\bibitem[\citeproctext]{ref-Huo2019}
...
\bibitem[\citeproctext]{ref-Ispoglou2018}
...
\bibitem[\citeproctext]{ref-Korobeynikov2024}
...
\bibitem[\citeproctext]{ref-MemZeroDataBarrier}
...
\bibitem[\citeproctext]{ref-Liljestrand2021}
...
\bibitem[\citeproctext]{ref-Liljestrand2019}
...
\bibitem[\citeproctext]{ref-Lipp2020}
...
\bibitem[\citeproctext]{ref-Lipp2018}
...
\bibitem[\citeproctext]{ref-McCall2019}
...
\bibitem[\citeproctext]{ref-MemSetProposal}
...
\bibitem[\citeproctext]{ref-Mushtaq2020}
...
\bibitem[\citeproctext]{ref-Mutlu2004}
...
\bibitem[\citeproctext]{ref-ObsdJunk}
...
\bibitem[\citeproctext]{ref-Osvik2005}
...
\bibitem[\citeproctext]{ref-Karger1974}
...
\bibitem[\citeproctext]{ref-saelo2021b}
...
\bibitem[\citeproctext]{ref-Schuster2015}
...
\bibitem[\citeproctext]{ref-Schwarz2017}
...
\bibitem[\citeproctext]{ref-Schwarz2019}
...
\bibitem[\citeproctext]{ref-Seacord2013}
Seacord, Robert C. 2013. \emph{Secure Coding in c and c++}. 2nd ed.
Addison-Wesley Professional.

\bibitem[\citeproctext]{ref-Serebryany2012}
...
\bibitem[\citeproctext]{ref-Sharma2014}
...
\bibitem[\citeproctext]{ref-Solar1997}
...
\bibitem[\citeproctext]{ref-Su2021}
...
\bibitem[\citeproctext]{ref-Thompson1984}
...
\bibitem[\citeproctext]{ref-wang2012undefined}
...
\bibitem[\citeproctext]{ref-Weber2021}
...
\bibitem[\citeproctext]{ref-Wingo2011}
...
\bibitem[\citeproctext]{ref-Wu2018}
...
\bibitem[\citeproctext]{ref-Yarom2014}
...
\bibitem[\citeproctext]{ref-Yoo2021}

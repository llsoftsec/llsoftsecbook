...
Layout Randomization (ASLR)} and \hyperref[cfi]{Control-Flow Integrity
(CFI)} measures were introduced. This interaction between offensive and
...
\item
  ...
  to maintain \hyperref[cfi]{Control-Flow Integrity (CFI)}.
\end{itemize}
...
memory safety or \hyperref[cfi]{Control-Flow Integrity} are necessary.
For example, in addition to the hardening of specific library functions,
compilers can also implement automatic bounds checking for arrays where
the array bound can be statically determined
...
flow transfer targets. (\citeproc{ref-Burow2017}{Burow et al. 2017})
compares a number of available CFI schemes based on the precision. For
...
the \hyperref[stack-buffer-overflow]{earlier stack buffer overflow
example} looks like:
...
instructions is PACStack (\citeproc{ref-Liljestrand2021}{Liljestrand et
al. 2021}), which chains together PACs in order to include the full
context (all of the previous return addresses in the call stack) when
signing a return address. \todospan{{Add more references to relevant
research
...
\todospan{{Mention more Pointer Authentication uses in later section,
and add link here
\href{https://github.com/llsoftsec/llsoftsecbook/issues/167}{\#167}}}
\todospan{{The use of pauth in arm64e or pauthabi should be explained in
...
Figure \hyperref[fig:non-control-data-attack]{4} shows the stack frame
...
\hyperref[fig:cache-indexing]{5}. The least-significant \(L\) bits,

...
predict\index{predict} which instruction needs to be executed after a
...
important for performance that most CPUs have multiple pieces of
\href{https://en.wikipedia.org/wiki/Branch_predictor}{branch
predictor}\index{branch
predictor} logic, such as:

\begin{itemize}
\tightlist
\item
  A component that predicts the outcome of a conditional branch: taken
  or not taken\index{taken branch}. The prediction is typically done
  based on the outcome of this and other branches in the recent past. In
  academic literature, this type of component is often called a Pattern
  History Table (PHT) \index{Pattern History Table (PHT)}.
\item
  A component that predicts the target of a taken branch, i.e.~the
  address of the next instruction after a taken branch. In academic
  literature, such a component is often called a Branch Target Buffer
  (BTB) \index{Branch Target
  Buffer}.
\item
  A component that is specialized to predict the next instruction after
  a function return instruction. Academic literature often calls this a
  Return Stack Buffer (RSB) \index{Return Stack Buffer}.
\end{itemize}
...
A number of attacks have been described over the past few years. A few
examples, categorized per branch predictor component they target, are:

\begin{itemize}
\item
  PHT: BranchScope (Evtyushkin et al.
  \protect\hyperlink{ref-Evtyushkin2018}{2018}), BlueThunder (Huo et al.
  \protect\hyperlink{ref-Huo2019}{2019}). These attacks infer whether a
  ...
  branch predictor entry (i.e.~aliases) with the targeted branch in the
  victim process. By measuring whether the branch in the spy process
  ...
  specific cryptographic kernels. It can also be used to break
  ASLR\index{ASLR}.
\item
  BTB: SBPA (Aciiçmez, Kaya Koç, and Seifert
  \protect\hyperlink{ref-Aciicmez2007}{2007}), BranchShadow (Lee et al.
  \protect\hyperlink{ref-Lee2017}{2017}). These earlier attacks are
  based on making a branch in the spy process alias in the BTB with a
  targeted branch in the victim process. They use methods such as timing
  difference, last branch records, instruction traces or performance
  counters to measure whether the branch in the spy process caused a
  specific state change in the BTB.
\item
  RSB: Hyper-Channel (Bulygin
  \protect\hyperlink{ref-Bulygin2008}{2008}). In this case, a spy
  process invokes \(N\) calls to fill up the return stack predictor.
  Then it lets the victim process execute. Then, the spy process can
  measure how many of its return stack entries have been removed from
  the RSB, by measuring the number of \(N\) returns that get
  mis-predicted. If the number of calls in the victim process is
...
Thompson (Thompson \protect\hyperlink{ref-Thompson1984}{1984}) provides
...
by various works such as (Wang et al.
\protect\hyperlink{ref-wang2012undefined}{2012}) (D'Silva, Payer, and
Song \protect\hyperlink{ref-d2015correctness}{2015}) (Du, Wu, and Mao,
\protect\hyperlink{ref-dusilent}{n.d.}). By reading the works of these
authors, one can see that even projects that went through careful
testing, such as Linux, FreeBSD or PostgreSQL, could not escape from
this class of vulnerabilities. To better understand them, this chapter
contains several examples of such vulnerabilities, their implications
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{leaq    }\BaseNTok{0xe0}\NormalTok{(%}\KeywordTok{rbp}\NormalTok{),%}\KeywordTok{rdi}
\NormalTok{xorl    %}\KeywordTok{esi}\NormalTok{,%}\KeywordTok{esi}
...
\BuiltInTok{movq}    \BaseNTok{0xe0}\NormalTok{(%}\KeywordTok{rbp}\NormalTok{),%}\KeywordTok{rdx}
\NormalTok{movl    }\BaseNTok{0xe8}\NormalTok{(%}\KeywordTok{rbp}\NormalTok{),%}\KeywordTok{edi}
\NormalTok{xorl    %}\KeywordTok{edx}\NormalTok{,%}\KeywordTok{edi}
\NormalTok{shll    }\DecValTok{$}\BaseNTok{0x10,}\NormalTok{%}\KeywordTok{eax}
\NormalTok{xorl    %}\KeywordTok{eax}\NormalTok{,%}\KeywordTok{edi}
\NormalTok{xorl    %}\KeywordTok{ebx}\NormalTok{,%}\KeywordTok{edi}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{leaq    }\BaseNTok{0xd8}\NormalTok{(%}\KeywordTok{rbp}\NormalTok{),%}\KeywordTok{rdi}
\NormalTok{xorl    %}\KeywordTok{esi}\NormalTok{,%}\KeywordTok{esi}
...
(\protect\hyperlink{ref-MemZeroBarrier}{2015}\protect\hyperlink{ref-MemZeroBarrier}{a}).
In case of using LTO, the buffer \texttt{s} is still vulnerable. For
this reason, Linux maintainers added a further hardening mechanism by
using a compiler barrier instead:
...
(\protect\hyperlink{ref-MemZeroDataBarrier}{2015}\protect\hyperlink{ref-MemZeroDataBarrier}{b}).
...
(\protect\hyperlink{ref-MemSetProposal}{2021}). This function requires
the compiler to not optimize the memory overwrite away. However it is
not trivial to implement such a functionality, as GNU presents in
(\protect\hyperlink{ref-GNUMemSet}{n.d.}). The information may be
present somewhere in the machine, even if it was erased from memory.
...
\leavevmode\hypertarget{ref-GNUMemSet}{}%
n.d.

...
\item
  ...
  (\citeproc{ref-Beer2020}{Beer 2020}), which describes a zero-click
...
(\citeproc{ref-Seacord2013}{Seacord 2013}) and
...
through a dispatcher gadget\index{dispatcher gadget}, which loads
pointers one after another from an array of pointers, and branches to
each one in return. The gadgets used must be set up so that they branch
or return back to the dispatcher after they're done. This is
demonstrated in figure \hyperref[fig:jop]{3}.
...
(COOP)\index{counterfeit object-oriented
programming (COOP)} (\citeproc{ref-Schuster2015}{Schuster et al. 2015})
is a code reuse technique that takes advantage of C++ \index{C++}
virtual function calls. A COOP attack takes advantage of existing
...
vfgadgets\index{vfgadget}. To chain vfgadgets together, the attacker
uses a ``main loop gadget'', similar to JOP's dispatcher gadget, which
is itself a virtual function that loops over a container of pointers to
C++ objects and invokes a virtual function on these objects.
(\citeproc{ref-Schuster2015}{Schuster et al. 2015}) describes the attack
in more detail. It is specifically mentioned here as an example of an
attack that doesn't depend on directly replacing return addresses and
code pointers, like ROP and JOP do. Such language-specific attacks are
...
(SROP)} (\citeproc{ref-Bosman2014}{Bosman and Bos 2014}). It is a
special case of ROP where the attacker creates a fake signal handler
...
rather than in machine registers\index{register}. Overwriting value in
...
(\citeproc{ref-Serna2012}{Serna 2012}).
...
coarse-grained\index{coarse-grained
CFI} or fine-grained\index{fine-grained CFI}. A coarse-grained CFI
scheme is one that uses a small number of large equivalence classes,
whereas a fine-grained scheme uses a larger number of smaller classes,
so that the possible branch targets from a given location are more
restricted (perhaps at a greater performance cost).
...
equivalence classes\index{equivalence class}. Each function is assigned
...
signature\index{function signature}.
...
Many CFI schemes check that a C++ \index{C++} virtual function call
...
Most CFI schemes do not protect against this, but arm64e\index{arm64e}
and pauthabi\index{pauthabi} do, as explained in the example below. This
is also explained in (\citeproc{ref-McCall2019}{McCall and Bougacha
2019}, slide 39-40).
...
next instruction is called the ``return address''\index{return address}.
...
Shadow stack\index{shadow stack} approaches store the return address on
...
\item
  ...
  \href{https://www.usenix.org/conference/usenixsecurity25/presentation/bajo}{Coroutine
  Frame-Oriented Programming(CFOP)}\index{Coroutine
  Frame-Oriented Programming (CFOP)}.
\item
  Procedure Linkage Table (PLT)\index{Procedure Linkage Table (PLT)} and
  the \href{https://en.wikipedia.org/wiki/Global_Offset_Table}{Global
  Offset Table(GOT)}\index{Global
  Offset Table (GOT)} often contain code pointers. One common way to
  protect these from being overwritten by an attacker is to make these
  tables
...
(PAC)\index{Pointer Authentication Code
...
\emph{raw pointer}\index{raw pointer}.
...
used to compute the PAC: a so-called key\index{key} and a
...
attacks\index{pointer substitution attack}, where a pointer that has
...
to C/C++ \index{C}\index{C++}programmers to help them detect
...
virtual address space (\citeproc{ref-Serebryany2012}{Serebryany et al.
2012}). Shadow memory is allocated in one contiguous chunk, which keeps
...
\item
  ...
  from writing to code memory (\citeproc{ref-Song2015}{Song et al.
  2015}), when multiple threads are involved. A more effective solution
...
approach include:
Flush+Flush\index{Flush+Flush}(\citeproc{ref-Gruss2016a}{Gruss, Maurice,
Wagner, et al. 2016}),
Flush+Prefetch\index{Flush+Prefetch}(\citeproc{ref-Gruss2016}{Gruss,
Maurice, Fogh, et al. 2016}),
Evict+Reload\index{Evict+Reload}(\citeproc{ref-Percival2005}{Percival
2005}), Evict+Time\index{Evict+Time}(\citeproc{ref-Osvik2005}{Osvik,
Shamir, and Tromer 2005}),
Reload+Refresh\index{Reload+Refresh}(\citeproc{ref-Briongos2020}{Briongos
et al. 2020}),
Collide+Probe\index{Collide+Probe}(\citeproc{ref-Lipp2020}{Lipp et al.
2020}), etc.
...
Two examples are BranchScope (\citeproc{ref-Evtyushkin2018}{Evtyushkin
et al. 2018})\index{BranchScope} and BlueThunder
(\citeproc{ref-Huo2019}{Huo et al. 2019})\index{BlueThunder}. These
...
Two examples are SBPA (\citeproc{ref-Aciicmez2007}{Aciiçmez, Kaya Koç,
and Seifert 2007})\index{SBPA} and BranchShadow
(\citeproc{ref-Lee2017}{Lee et al. 2017})\index{BranchShadow}. These
earlier attacks are based on making a branch in the spy process alias in
the Branch Target Buffer (BTB\index{BTB}) with a targeted branch in the
victim process. They use methods such as timing difference, last branch
records\index{last branch record}, instruction
...
One example is Hyper-Channel (\citeproc{ref-Bulygin2008}{Bulygin
...
Meltdown\index{Meltdown} (\citeproc{ref-Lipp2018}{Lipp et al. 2018})
attacks in 2018 started a period in which a large number of transient
attacks were discovered and published. Most of them were given specific
names, such as ZombieLoad, NetSpectre, LVI, Straight-line Speculation,
...
fully eliminate dead stores (\citeproc{ref-MemZeroBarrier}{{``Lib:
Memzero\_explicit: Use Barrier Instead of {OPTIMIZER\_HIDE\_VAR}''}
2015}). In case of using LTO, the buffer \texttt{s} is still vulnerable.

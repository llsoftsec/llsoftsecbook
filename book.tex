...
\item
  ...
  prediction is typically a history-based branch prediction, i.e., based
  on the outcome of this and other branches in the recent past.
\item
...
\item
  ...
  predictor\index{conditional branch direction
  predictor}: BranchScope (Evtyushkin et al.
  ...
  ASLR\index{ASLR}.
\item
  Branch Target Predictor\index{Branch target predictor}: SBPA
  (Aciiçmez, Kaya Koç, and Seifert
...
\item
  Return address predictor\index{return address predictor}:
  Hyper-Channel (Bulygin \protect\hyperlink{ref-Bulygin2008}{2008}). In
  this case, a spy process invokes \(N\) calls to fill up the return
  stack predictor. Then it lets the victim process execute. Then, the
  spy process can measure how many of its return stack entries have been
  removed from the RSB, by measuring the number of \(N\) returns that
  get mis-predicted. If the number of calls in the victim process is
